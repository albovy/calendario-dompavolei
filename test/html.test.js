...
test('crearHtml: el escudo del club (escudo.png) como data URI, el último de los datos', () => {
  ...
  try {
    ...
    const op = opciones({ salidas: SALIDAS, duracion: 90, urlPublicada: 'https://example.github.io/calendario/calendario.ics' });
    ...
    assert.equal(vacio, '');
    assert.deepEqual(resto, sinEscudo);
    // Sin escudo.png, "".
    assert.equal(datosDe(crearHtml({ ...op, rutaEscudo: join(dir, 'no-existe.png') })).escudo, '');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('crearHtml: un escudo que no se puede leer avisa y la página sale sin él', (t) => {
  const dir = mkdtempSync(join(tmpdir(), 'escudo-'));
  try {
    const avisos = [];
    t.mock.method(console, 'log', (texto) => avisos.push(texto));
    // Una carpeta en lugar del archivo: no se puede leer como imagen.
    assert.equal(datosDe(crearHtml({ ...opciones(), rutaEscudo: dir })).escudo, '');
    assert.equal(avisos.length, 1);
    assert.match(avisos[0], /^ {2}! No se pudo leer el escudo \(.+\); la página sale sin él\.$/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
